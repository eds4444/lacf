...
$(function() {
    ...
    $(window).resize(function() {
        var i = $(".about_4, .about_5").index(first);
        if ($(window).width() <= "568" && !i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first);
            first.insertAfter(second);
            second.insertAfter(a);
            a.remove()
        } else if ($(window).width() >= "568" && i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first);
            first.insertAfter(second);
            second.insertAfter(a);
...
$(function() {
    ...
    var second2 = $(".about_4");
    $(window).resize(function() {
        var i = $(".about_2, .about_4").index(first2);
        if ($(window).width() <= "568" && !i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first2);
            first2.insertAfter(second2);
            second2.insertAfter(a);
            a.remove()
        } else if ($(window).width() >= "568" && i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first2);
            first.insertAfter(second2);
            second.insertAfter(a);
            a.remove()
        }
    }).resize()
...
$('#showallques').click(function(){
  if (!$(this).data('status')) {
    $(this).html('<p>Скрыть вопросы</p>');
    $(this).data('status', true);
  }
  else {
    $(this).html('<p>Смотреть все вопросы</p>');
    $(this).data('status', false);
    $('html,body').animate({scrollTop:$('.faq__body').offset().top+"px"},{duration:1E3});
  }
});

    $(document).ready(function() { 
	      $("A#showallques").click(function() { 
	        // Отображаем скрытый блок 
	        $(".hidden_faq").toggleClass('show');
    ...
    $(document).ready(function() { 
    	if($(window).width() < 767){
	        ...
	        // Отображаем скрытый блок 
	        $(this).siblings(".company__info").fadeToggle(); 
	        $(this).siblings("p.slide__info").toggleClass('show');
	        return false; // не производить переход по ссылке
	      	},  
      function() { 
        // Скрываем блок 
        ...
        return false;
		    ...
		    $('.full_text').click(function () {
			    ...
			    $('.popup__fulltext').addClass('active');
			    $('html').addClass('hidden');
			});
			$('.full_text').attr('onclick', '$(".popup__fulltext .dashedWrap").html($(this).parent(".reviews__body").find("p.slide__info").html());'); 
		}
    });

    $("#showallques.active").on("click", function(){
      setTimeout(scrollTop, 2000);
      $('#showallques').stop().animate({ scrollTop: $(this.hash).offset().top - fixed_offset }, 1000);
    });
...
$(function() {
    ...
    var second3 = $(".about_3");
    $(window).resize(function() {
        var i = $(".about_2, .about_3").index(first3);
        if ($(window).width() <= "5000" && !i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first3);
            first3.insertAfter(second3);
            second3.insertAfter(a);
            a.remove()
        } else if ($(window).width() > "5000" && i) {
            var a = $("<abracadabra/>");
            a.insertAfter(first3);
            first3.insertAfter(second3);
            second3.insertAfter(a);
...
$("body").on('click', '[href*="#"]', function(e){
  ...
  //попап
  $('.popupBtn, .popup > .close, #pop .overlay, [data-wrap-sent], #pop').click(function () {
...
$('[data-popup="popup__pay"]').click(function() {
  $('html').addClass('trec');
});
...
/* Функция для - animate height: auto */
  function autoHeightAnimate(element, time) {
    var paddings = 75; //отступы в блоке с ответом
    var curHeight = element.height(), // Получить высоту по умолчанию
      autoHeight = element.css('height', 'auto').height() + paddings; // Сделать высоту auto
...
var lastItems = $(".accordion:first-child h3");
lastItems.slice(lastItems.length - 5).addClass("hidden_faq");
var lastItems = $(".accordion h3");
lastItems.slice(lastItems.length - 5).addClass("hidden_faq");
var lastItems = $(".accordion > div");
lastItems.slice(lastItems.length - 5).addClass("hidden_faq");
var lastItems = $(".accordion:first-child > div");
lastItems.slice(lastItems.length - 5).addClass("hidden_faq");
